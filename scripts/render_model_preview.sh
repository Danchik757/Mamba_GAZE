...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 MODEL_NAME [extra args for tools/render_model_preview.py]" >&2
  exit 1
fi
...
python "${REPO_ROOT}/tools/render_model_preview.py" \
  --model "${MODEL_NAME}" \
  --mesh-dir "${MESH_DIR}" \
  --json-dir "${JSON_DIR}" \
  --output-image "${PREVIEW_DIR}/${MODEL_NAME}_frame0_preview.ppm" \
  "$@"
